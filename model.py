...
class SoftHebbConv2d(nn.Module):
    def __init__(
        ...
        weight = torch.empty((out_channels, in_channels // groups, *self.kernel_size)).requires_grad_(False).to(device)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            CiCjCk = self.Cj[:, None, None, None] * self.Ci[None, :, None, None] * self.Ck1[None, None, :, None] * self.Ck2[None, None, None, :]
            ...
            self.weight = self.weight - delta_weight * CiCjCk # norm_diff.view(-1, 1, 1, 1) # store in grad to be used with common optimizers
...
class DeepSoftHebb(nn.Module):
    def __init__(self, device = 'cpu', in_channels=1, dropout=0.0):
        super(DeepSoftHebb, self).__init__()
        # block 1
        self.bn1 = nn.BatchNorm2d(in_channels, affine=False).requires_grad_(False)
        self.conv1 = SoftHebbConv2d(in_channels=in_channels, out_channels=96, kernel_size=5, padding=2, t_invert=1, device=device)
        self.activ1 = Triangle(power=0.7)
        # self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.pool1 = nn.MaxPool2d(kernel_size=4, stride=2, padding=1)
        # block 2
        self.bn2 = nn.BatchNorm2d(96, affine=False).requires_grad_(False)
        self.conv2 = SoftHebbConv2d(in_channels=96, out_channels=384, kernel_size=3, padding=1, t_invert=0.65, device=device)
        self.activ2 = Triangle(power=1.4)
        self.pool2 = nn.MaxPool2d(kernel_size=4, stride=2, padding=1)
        # block 3
        self.bn3 = nn.BatchNorm2d(384, affine=False).requires_grad_(False)
        self.conv3 = SoftHebbConv2d(in_channels=384, out_channels=1536, kernel_size=3, padding=1, t_invert=0.25, device=device)
        ...
        self.classifier = SoftHebbLinear(24576, 10, device=device, t_invert=1)
